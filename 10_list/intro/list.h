...
template <typename T> struct Node { // Node Structure
  T data;
  Node *next;
  ...
  friend std::ostream &operator<<(std::ostream &out, Node<T> *n) { return out << n->data; }
};
...
public:
    ...
    bool operator==(LinkedList<T> &ll) {
        if (m_size != ll.m_size) return false;
        _Node *e2 = ll.head;
        for (_Node *tmp = head; tmp != nullptr; tmp = tmp->next) {
            if (tmp->data != e2->data) return false;
            e2 = e2->next;
        }
